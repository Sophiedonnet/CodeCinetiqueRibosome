...
alpha_ND_V <- transfo_Gamma_Param(70,150)[1]
beta_ND_V <- transfo_Gamma_Param(70,150)[2]

alpha_ND_R <- transfo_Gamma_Param(85,100)[1]
beta_ND_R <-transfo_Gamma_Param(85,100)[2]
...
pi_QD <- 0.000000001# probability of quick death
lambda_QD <- 1;

#- all Param
param_true <-  c(alpha_ND_V,beta_ND_V, alpha_ND_R,beta_ND_R,lambda_c,lambda_e,lambda_QD,pi_QD)
log_param_true <- param_true
log_param_true[1:7] <- log(param_true[1:7])
log_param_true[8] <- -log(1-param_true[8]) + log(param_true[8])
#mu_true <-param_true
...
#------- Natural death
T_R_C <- rGammaCensored(n_C,alpha_ND_R,beta_ND_R,Tmax)
T_V_C <- rGammaCensored(n_C,alpha_ND_V,beta_ND_V,Tmax)

par(mfrow=c(2,2))
hist(T_V_C,freq = FALSE,nclass = n_C/10,main="Natural Death")  
curve(dGammaCensored(x,alpha_ND_V,beta_ND_V,Tmax),add=TRUE,col='green')
hist(T_R_C,freq = FALSE,nclass = n_C/10,main="Natural Death")  
curve(dGammaCensored(x,alpha_ND_R,beta_ND_R,Tmax),add=TRUE,col='red')

plot(ecdf(T_V_C))
curve(pGammaCensored(x,alpha_ND_V,beta_ND_V,Tmax),add=TRUE,col='green')
plot(ecdf(T_R_C))
curve(pGammaCensored(x,alpha_ND_R,beta_ND_R,Tmax),add=TRUE,col='red')
...
paramsChains = list(nMCMC=10000,rho=rep(1,8))
paramsChains$nBurnin <- paramsChains$nMCMC/10
paramsChains$withQD  = TRUE

paramsChains$paramsToSample = c(1:6) #,5:8) #


log_param_init = c(0,-1,0,-1,-2,-1,1,0)
log_param_init[1] <- log(mean(mydata$T_Contr_V)^2/var(mydata$T_Contr_V))
log_param_init[2] <- log(mean(mydata$T_Contr_V)/var(mydata$T_Contr_V))
log_param_init[3] <- log(mean(mydata$T_Contr_R)^2/var(mydata$T_Contr_R))
log_param_init[4] <- log(mean(mydata$T_Contr_R)/var(mydata$T_Contr_R))


hyperparams= list(mean=rep(0,8) ,sd = rep(3,8))
for (i in 1:8){
...
mySample <- my_mcmc_marg_Gamma_onechain(mydata,log_param_init,hyperparams,paramsChains)
...
abs <- seq(0,mydata$Tmax_R,len= 1000)
param_estim <- log_param_estim
param_estim[1:7] <- exp(log_param_estim[1:7])
param_estim[8] <- 1/(1+exp(-log_param_estim[8]))

P <- computationToPlotCurves(abs, param_true,mydata$k,mydata$kprime,Tmax=list(V=mydata$Tmax_V,R = mydata$Tmax_R))
P$comp <- "True"
Pestim <- computationToPlotCurves(abs, param_estim,mydata$k,mydata$kprime,Tmax=list(V=mydata$Tmax_V,R = mydata$Tmax_R))
Pestim$comp <- "Estim"
allP <- rbind(P,Pestim)

levels(P$Curves)
Ptemp <- allP #%>% filter(Curves %in% c("5. Final model"))
...
curve(dOurModel(x,param_true,mydata$k,mydata$kprime,color='green',Tmax=mydata$Tmax_V),add=TRUE,col='green',lwd=2,lty = 1)
curve(dOurModel(x,param_estim,mydata$k,mydata$kprime,color='green',Tmax = mydata$Tmax_R),add=TRUE,col='orange',lwd=2,lty = 2)
...
curve(dOurModel(x,param_true,mydata$k,mydata$kprime,color='red',Tmax = mydata$Tmax_R),add=TRUE,col='red',lwd=2,lty = 1)
